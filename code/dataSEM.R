...
# Subset data for covariance matrix
sem2 <- subset(d, select = c(c1a:c1r, f1a:f1g, f2a1, f2b1, f2c1, f2d1, f2e1, f2f1, f2g1, f2a2, f2b2, f2c2, f2d2, f2e2, f2f2, f2g2, f5a:f5c, d1h:d1j, g2a:g2d, h1a:h1c, neighbors_f2a1:near_03f))

# Assign NA to missing values that were coded 99 or 999:
sem2[sem2 == 99 | sem2 == 999] <- NA  

# Get covariance matrix
cov <- cov(sem2, use = "pairwise.complete.obs")
!cov <- cov(sem2, use = "complete.obs") # missing deleted casewise

# Write covariance matrix to csv file
write.csv(cov, 'data/covariance.csv')
