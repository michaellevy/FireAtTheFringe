...
d2 = readRDS('../data/derived/dataVImodel.RDS') %>% as.data.frame()
...
We condition a multilevel Bayesian model of defensible space behavior, which we later use to generate the population of agents. The outcome variable is the number of defenisble space behaviors adopted, out of four, instantiated as a binomial random variable. The model contains varying intercepts at the level of town ($\alpha_{town}$) to pool information across towns in determining the effect of social forces on the base-rate of adoption. The other predictors are housing density ($D$), beliefs about the effectiveness of defensible space practices ($E$), and beliefs about risk associated with wildfire ($R$). We employed weakly regularizing priors, $\mathcal{N}(0, 1)$, for all continuous predictors to avoid overfitting. The model takes the form:

$$
\begin{aligned}
N_i &\sim binomial(4, p_i) \\
logit(p_i) &= \alpha_{town[i]} + \beta_{D} ln(D) + \beta_{E} E + \beta_{R} R + \beta_{R} R \\
...
Figure 1 shows the distribution of number of defensible space behaviors, out of four possible, adopted per household by town. Across all towns, the modal number of behaviors adopted is four,the median is two, and the mean `r round(mean(d2$numBehaviors), 2)`. Figure 1 also shows the number of survey responses in our dataset from each town, which are roughly proportional to the number of homes in the WUI in each town.
...
```{r prediction check, fig.cap = "For 30 survey respondents sampled randomly, red diamonds are empirical number of defensible space behaviors, and violin plots reflect the distribution of model-implied behaviors for simulated households with the same set of predictor values."}
set.seed(3957)
sims = sim(m, data = d2)
...
```{r map DS observed and predicted}
