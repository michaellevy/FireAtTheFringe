...
globals [
  ...
  BN  ;; multiplier from beliefs to norms
]
...
;  set num-neighbors 10  ;; number of neighbors on which each homeowner determines local norms
  ...
  ask patches [
    ...
    if pxcor = min-pxcor
      [ set pcolor red ]  ;; red patches are aflame, red - 3.5 patches already burned
  ]
  ...
  ask turtles [
    ...
    set color 10 + 10 * ds  ;; set turtles' color in [10, 20] so that's darkred when low adoption prob; lightred when high prob
    set size 6
    set shape "house"
  ]

  implement-ds  ;; firewise turtles clear their properies!
